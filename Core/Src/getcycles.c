...
int getcyclesmain(int argc, char* argv[]) {
...
#if !defined(__AVR__)
      printf("%6.1f %6.1f\n", factor * cycles[i][0] / bytes + 0.05, // @suppress("Float formatting support")
...
#if !defined(__AVR__)
      printf("| %5.1f ", factor * cycles[i][0] / bytes + 0.05); // @suppress("Float formatting support")
#else
      printf("| %5" PRIu32 " ",
             (uint32_t)(factor * cycles[i][0] / bytes + 0.05));
#endif
